...
export class ClientsController {
  ...
  @Get() findAll(@CurrentTenant() t: TenantPayload, @Query('search') search?: string) {
    return this.service.findAll(t.id, search)
  }

  @Get(':id') findOne(@Param('id') id: string, @CurrentTenant() t: TenantPayload) {
    return this.service.findOne(id, t.id)
  }

  @Get(':id/history') history(@Param('id') id: string, @CurrentTenant() t: TenantPayload) {
    return this.service.getHistory(id, t.id)
  }

  @Post() create(@Body() dto: CreateClientDto, @CurrentTenant() t: TenantPayload) {
    return this.service.create(dto, t.id)
  }

  @Patch(':id') update(@Param('id') id: string, @Body() dto: Partial<CreateClientDto>, @CurrentTenant() t: TenantPayload) {
